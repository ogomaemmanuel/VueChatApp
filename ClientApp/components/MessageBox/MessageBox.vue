<template>
    <div class="chat-message-box">
        <ChatMessageBoxHeader :show-body.sync="showBody"
                              :online-user="onlineUser" />
        <div v-show="showBody"
             class="chat-message-box-body">
            <div ref="messages"
                 class="messages">
                <div v-for="filtedMessage in fiteredChatMessages"
                     :key="filtedMessage.id">
                    <ReceivedMessage v-if="filtedMessage.toId==auth.userDetails.id"
                                     :message="filtedMessage" />
                    <SentMessageCard v-if="filtedMessage.fromId==auth.userDetails.id"
                                     :message="filtedMessage" />
                </div>
            </div>
            <MessageBoxFooter :online-user="onlineUser" />
        </div>
    </div>
